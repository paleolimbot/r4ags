...
ggplot(warwick_norm, aes(x = element, y = reference, group = sample_id)) +
  ...
  labs(x = NULL, y = "Concentration / Reference concentration", col = "Sample ID") +
  scale_y_log10() +
  theme_bw()
```
...
For more on how to use the [ggtern package](http://ggtern.com), see the [journal article](https://www.jstatsoft.org/article/view/v087c03). Note that the ggtern package breaks other plots (including those from the `tidypaleo` package), so you might want to generate these plots separately!
